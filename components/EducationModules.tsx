...
const StudyMaterialModal: React.FC<{
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center p-4" aria-modal="true" role="dialog">
        <div className="bg-[--brand-bg] dark:bg-[--dark-surface] rounded-3xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col animate-fade-in-up">
            ...
            <div className="p-6 overflow-y-auto space-y-6">
                <div className="p-4 bg-purple-500/10 rounded-2xl border border-purple-500/20">
                    <h4 className="font-semibold text-purple-800 dark:text-purple-200">Context</h4>
                    <p className="text-sm text-purple-700 dark:text-purple-300 mt-1">{module.studyMaterial.introduction}</p>
                </div>
                <div>
                    <h4 className="text-lg font-semibold mb-3">Key Safety Points</h4>
                    <div className="space-y-4">
                        {module.studyMaterial.keyPoints.map((point, index) => (
                            <div key={index} className="flex items-start">
                                <CheckCircleIcon className="h-6 w-6 text-purple-500 mr-3 mt-1 flex-shrink-0" />
                                <div>
                                    <h5 className="font-semibold">{point.title}</h5>
                                    <p className="text-[--brand-slate]">{point.detail}</p>
                                </div>
                            </div>
                        ))}
                    </div>
  ...
  return (
    <div>
      <div className="text-center mb-12">
        <h1 className="text-4xl font-bold sm:text-5xl"><span className="underline-squiggle">Disaster Study Modules</span></h1>
        <p className="mt-4 text-lg text-[--brand-slate]">Choose a module to learn about specific disasters and enhance your preparedness knowledge.</p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {DISASTER_MODULES.map((module) => (
          <div key={module.type} className="bg-white dark:bg-[--dark-surface] rounded-3xl soft-shadow soft-shadow-hover overflow-hidden flex flex-col">
            <div className="p-6 bg-[--brand-purple] flex justify-center items-center h-32">
      ...
      {selectedModule && ReactDOM.createPortal(
        <StudyMaterialModal module={selectedModule} onClose={() => setSelectedModule(null)} />,
        document.body
      )}
    </div>
  );
